...
- **🚧 HUD redesign INTERROMPIDO no checkpoint:** user salvou `assets/HUD.png` (1981×794) — dashboard completo: fuel células laranja + badge bomba, radar central c/ moldura metal, graviton células roxas + badge átomo, placas de label (2 em cima, score embaixo), 4 ícones pixel verdes soltos no topo (vaca/fazendeiro/espantalho/celeiro). Análise: RGB **SEM alpha** (fundo preto baked — tratar antes de usar). Próximo: mapear regiões → propor encaixe → **CONFIRMAR com user** antes de implementar
- **Backlog:** wire dos 28 SFX cartoon quando chegarem (6 hooks novos: ufo_hover_loop, moos por estado, bull_snort, farmer_hey, low_fuel, birds/crickets por TOD, ui_click + pitch/vol random) · 5ª faixa de música (player escala sozinho) · interface nova pro score (user vai puxar referência) · crateras por deformação de heightfield (beam cava) · mobile/WASM · grass verlet · vsync toggle · publicar repo Bevy no GitHub (local-only hoje — decisão do user) · upgrade Bevy 0.15→0.19 (quando polish acalmar) · validação user pendente: 2026-07-11 noite (chuva nova + respingos + blips dots + legendas) + terraços/medo do vazio/queda centrada de 07-11 + itens de 07-07/07-06 ainda não vistos
