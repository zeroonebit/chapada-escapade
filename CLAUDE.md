...
- **Backlog:** crateras por deformação de heightfield (beam cava) · mobile/WASM · barrel · grass verlet · sons (adiado pelo user) · vsync toggle · publicar repo Bevy no GitHub (local-only hoje — decisão do user) · upgrade Bevy 0.15→0.19 (quando polish acalmar) · validação user pendente: i18n toggle + tradução do código (2026-07-08) + TUDO de 2026-07-07 (beam sem flicker, aspect ratio, torres/náufragos, aba ASSETS, tecla B, painel LIVE, nearest) + de 07-06 (hanabi runtime, neve v2, balões, sombras TOD, radar-minimapa)
