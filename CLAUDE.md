...
- **Roadmap alpha (docs/design/ALPHA_REVIEW.md §7):** ✅quota+escalada → tutorial 10 passos → tela de achievements + 5 quests + rare spawns → bloqueadores técnicos (AssetPlugin relativo, panic→log, fullscreen/vsync, reset save) → balance pass → zip pros testers
- **Backlog:** wire dos 28 SFX cartoon quando chegarem (6 hooks novos: ufo_hover_loop, moos por estado, bull_snort, farmer_hey, low_fuel, birds/crickets por TOD, ui_click + pitch/vol random) · 5ª faixa de música (player escala sozinho) · veredito do cockpit v2 (assets/refs/cockpit_v2_candidate.png — vs HUD clássico) · scale_audit --apply-category (aguardando ok do user) · quests runtime + garagem (coins já acumulam) · tema KIT polish (task #31) · crateras por deformação de heightfield (beam cava) · mobile/WASM · grass verlet · vsync toggle · publicar repo Bevy no GitHub (local-only hoje — decisão do user) · upgrade Bevy 0.15→0.19 (quando polish acalmar) · validação user pendente: clusters/occupancy/ondas direcionais/critters de 07-12 + chuva nova/respingos de 07-11
