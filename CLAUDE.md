...
- Repo: `H:/Projects/Bevy/ChapadaEscapade/` — port 1:1 do Phaser em **Rust+Bevy 0.19** (migrado da 0.15 em 2026-07-14), jogo desktop completo
...
- **i18n + tradução total (2026-07-08, 16 commits `921d69a`→`80ea4ef`):** toggle **LANGUAGE ENG/PTBR** no CONFIGS→CONTROLS com **default EN** (`DebugConfig.lang` + helper `cfg.tr(en,pt)`/`cfg.is_pt()`) · textos de jogo bilingues (tutorial, game over/vitória, pause, diálogo tecla B, label FUEL) · quips 1:1 dos pools EN do Phaser (`quips.rs` `pool(pt)`) · **CODEBASE 100% EN pra equipe de porting**: 26 `.rs` + 2 shaders WGSL (`terrain_proc`/`water_proc`), comentários e strings de dev traduzidos, identificadores mantidos · **15 batches** commitados um a um, `cargo check` limpo em cada · metodologia: scripts Python de substituição exata + re-scan grep estrito de palavras PT (não só acento) até zerar · "migração pra pasta certa" = no-op (repo já mora em `H:/Projects/Bevy/ChapadaEscapade`, cópia única); publicar remote fica esperando decisão do user (local-only por design)
- **CRT + heat-haze + buraco negro + terraços + radar vivo (2026-07-11, 26 commits `6853bca`→`4f165b4`):** **CRT post-process** (1º do projeto: ViewNode no Core3d, barrel quártico + scanlines finas + vignette + aberração + grão + edge blur SEM borda preta, HUD indentado via `crt_warp`, 6 sliders VFX) · **beam = heat-haze field** (câmera HDR+Bloom, `beam_field.wgsl` refrata o fundo na elipse projetada + núcleo HDR florescendo no bloom; discos mesh DELETADOS; máscara da nave só no haze) · **UI Phaser total** (VT323 default do egui + palette exata `#00ff55/#aaffcc/#001a08`, HUD X/Y, +100 popup, FUEL flash, ufo_height slider, ícone do exe via winresource+winit, CONFIGS 86%×72%) · **terreno com identidade**: TERRAÇOS praia 0<grama 0.30<terra 0.62 (clamp relativo ao degrau, âncoras de curral/NPC corrigidas) + BAIXADAS orgânicas fBm 16-30u (piso -2.2, causa raiz: piso -0.26 engolia tudo) + PICO nevado intransponível c/ anel de nuvens + **BURACOS NEGROS 1-3/seed** (poço de gravidade → horizonte de eventos → espiral no centro de massa encolhendo → game over; névoa própria) + NPCs com MEDO do vazio (spawn+steering) · avental de oceano virou ANEL (quad 7× por baixo da ilha alagava as baixadas) · **névoa viva** (campo de nuvens no MUNDO 57×33, drift INTEGRADO — fix teleporte no crossfade — sliders density/coverage/size/speed) · **radar-instrumento** (minimapa 400² bilinear+warp+hillshade+blend largo, marcadores pico/abismo, zoom dial 0.5/0.75/1.0 vencedor do A/B, blips = mini SPRITES, névoa zoom-aware no disco) · lições: cap 16 params de system (tuple-param 2×), `git add` com path ignorado vaza o stage pro commit seguinte (ícone entrou no `87f6cec`)
- **ÁUDIO completo + polish radar/chuva (2026-07-11 noite, 10 commits `0cf0e1d`→`fab507f`):** **som do zero via bevy_audio built-in** (Cargo features wav+mp3): `src/audio.rs` SfxPlugin — 12 one-shots (Grab/Drop/Deliver/BurgerReady/Collect/Shot/Hit/Explosion/Thunder/Fall/GameOver/Victory) pegando carona nos eventos existentes + loops beam/chuva/vento com lerp · `tools/gen_sfx.py` 15 WAVs procedurais (Python stdlib, seed 1991) · **música dinâmica**: 4 MP3s AI em `assets/audio/` com crossfade por estado (menu no splash, dia/noite por TOD idx 3|4), pasta escaneada em runtime · **music player no CONFIGS AUDIO** (⏮/⏭ + combo AUTO/manual + sliders music/sfx) · bug: IA renomeou faixa noturna → const errada deixaria a noite muda · prompts entregues: música + **28 SFX cartoon** Looney Tunes (user gerando; híbrido c/ Kenney CC0 depois) · **radar**: blips → pontinhos coloridos de volta (sprites ficaram ENORMES; só o disco fica sprite 20px), metade do tamanho (4.5/6.0), burgers prontos = blip ROXO, círculo-legenda nas caixas do HUD (marrom/branco/amarelo/vermelho/roxo), score box -22%, roda do mouse = zoom · **chuva forte/acesa/molhada**: 420 gotas maiores+claras (Add brilha de noite), aterrissam no RELEVO REAL (antes afundavam nas chapadas), respingos = pool 90 anéis expand+fade (textura em código, depth_bias -55) · splash PLAY/TUTORIAL em VT323 explícita
- **Mega-sessão pré-alpha (2026-07-12, ~30 commits `14e9005`→`73f55e4`):** **CORE LOOP NOVO**: vitória por QUOTA 30 burgers (check antigo stale 6 torres vs 10) + escalada por entrega (torres +80%, farmers +50%) + torres=objetivo lateral pago · **COMBO** (+2 vacas no curral cheio = bebida+batata, 1.5×pts/1.75×fuel) · **PROGRESSÃO**: save.json + 50 achievements (tabela gerada de docs/design/ACHIEVEMENTS.md via tools/gen_achievements_table.py) + tracker wave-1 + toasts c/ ícone — comprovado em jogo · **ÁGUA em 3 atos**: EDT euclidiano (BFS=octógono) → domain warp ±1.8cel na geometria (platôs de célula) → rios Catmull-Rom + lagoas metaball + ondas DIRECIONAIS c/ vento (fase dist-à-costa = "caleidoscópio" em lago redondo) · **SHORE-LIFE**: sapo/pato/égua completos (spawn beira d'água; PixelLab: jobs falham silencioso, grupos hasheados, 502 duplica, disco cheio) · **SCATTER 2.0**: clusters âncora→satélites no scatter.json (14 sets, regra do galão do Phaser generalizada) + Occupancy spatial-hash global (fim da sobreposição) + trim -30% (7099→5425) + 4 trucks coloridos · **HUD**: dash 46%, score curvado c/ quinas pill no verde forte, células off da própria arte, fuel espelhada, labels s/ %, dial/nomes maiores, tema A/B (CLASSIC venceu), aba AUDIO dedicada (player estava enterrado na CONTROLS) · **TOOLING**: ⚖ compare no SIZES c/ px/u, tools/scale_audit.py (--apply-category), world presets 1×/3×/5× · **ARTE ~250 gens (saldo ~810)**: 50 ícones achievements em 1 call, 8 raros+moedas, hangar, combo+3 gags, 4 pickups, cockpit v2 candidato (create-ui-asset c/ pieces) em assets/refs/ · **FIXES**: touro frames rosa, véus TOD capados 0.32/0.42, chuva RAIN_WIND 3.8, rajada dessincronizada, torres→ESPANTALHOS · **docs/design/ALPHA_REVIEW.md** (P1/P2/P3 + 15 achievements + 7 quests + tutorial 10 passos + checklist alpha: bloqueador AssetPlugin path absoluto) · workflow novo: compilar+abrir+self-test automático (memória)
- **Alpha §7.2-7.3 + cockpit FinalHud v4 + mundo 1× (2026-07-13, ~20 commits `bfaebae`→`5844473`):** **tutorial 10 passos** (+RADAR legenda, +HAZARDS com CamPeek 1.4s pro abismo, KillTower tático, SKIP na caixa — hold-ESC colide com pause, graduate no save + splash first-run nudge) · **§7.3**: aba AWARDS (grid 50, Hidden="???", quadro FinalPanelH com grid dentro do vidro via allocate_new_ui) + `quests.rs` (pool 5, roll 3/partida, coins no save, Contract Sweep +50%, rare items golden_burger/alien_artifact com coleta via beam + blip estrela branca) · **cockpit FinalHud do user (4 versões na sessão)**: pipeline UNBAKE (alpha por silhueta fechada + flood fill — o xadrez vinha baked em RGB; metal claro engana flood, 3 iterações), cockpit 2056×541 com counters baked (ícone sprite 1.4× + número só), 12 células/barra, PROPULSION REACTOR FUEL / TRACTOR BEAM PULSE, dash 44% + sombra baked, **joystick VIVO** (crop feathered desloca com a velocidade) · **FinalPanelH ×2**: MAP MISSIONS esq + SHIP LOG dir, clique esconde, Q/M alterna, aba UI player-visible · **radar**: blips clusterizados (bins 12px, r∝√count) + LENTE fisheye r^1.45 (7 anéis, slider) + cores dos LEDs da arte · **mundo 1× TRAVADO** (3×/5× fora; minimapa ilha inteira, zoom removido) · **torpedos perseguidores** (5-7s; vermelho = avançado 2×10s em pinça; 18 u/s cd 1.4-2.6) · **habitats** (farmyard nos currais/highlands/shore/open range + coleira — cão/gato/cabra já existiam do Toy Box, 0 gens, saldo 884) + buffer de água (spawn 3+ células, push gradiente) · **beam-bússola** substitui a seta (tint vermelho→cyan por distância+direção do curral) · remix áudio (chuva↓ trovão↑ hum↑) · scale_audit --apply-category APLICADO (151 sizes)
- **Asset QA + escala rel/abs + orientação + aba ASSETS (2026-07-13 madrugada+tarde, 14 commits `df0c7be`→`8171bfd`):** **ASSET TOUR** (`asset_tour.rs`, env `CHAPADA_TOUR=1`): teleporta o disco a 1 instância de cada asset, condições de estúdio, screenshot nativo → `tools/saves/tour` + `tour_montage.py` contact sheets; 3 corridas (v1 morreu na queda da borda→`!tour.active` no rim, v2 saiu com chuva→WeatherMix forçado por frame, v3 150 shots limpos) · **CLUSTERS SEMÂNTICOS** (26 rules): sentinel "corral" (corral_spots vira âncora) → caveira/coxo/casinha/outdoor; fogueira+cooler; pato na cachoeira; sino→farmhouse · **CRITTERS VISÍVEIS**: goat/dog/cat/mare 4-8 (scaled_count ×0.6 no 1× deixava 1-2) — 10 espécies nos habitats certos, 0 gens · **`scale_density_audit.py`**: RELATIVA (size=quad) × ABSOLUTA (densidade texels/u: farmer 62, toybox 21, landmarks 14) × EFETIVA (fração do canvas — margem transparente mente); fixes SCARECROW_QUAD 7→5, stable/fence/trucks · **`orientation_audit.py`**: simetria H/V + CoM CONSCIENTE DE CATEGORIA (billboard nunca rola, decal plano rola se simétrico, MIRROR_H tomba só pros lados) → `orientation.json`, scenery aplica via UV-flip (escala -1 back-face culla) · **DECAIS casam cor com terreno** (`decal_terrain_class` média RGB: milho→terra), campos upright · **TORRE = espantalho clássico** tb_scarecrow (mech engavetado), torpedos mantidos · **HUD sombra 3px** (silhueta preta deslocada) · **ABA ASSETS**: master "todos" por categoria + categoria ANIMALS (cfg.critters_off, respawn ao vivo) + categoria ENEMIES (farmer/tower, respawn ao vivo) no TOPO, thumbs do frame sul · **BUG jogo não esvaziava**: cluster satellites ignoravam `enabled` (agora gateados) · **leite confuso**: currais já bakeiam props → cluster parou de duplicar · **joystick só a bola** (crop 106×106) · **FRIENDLY-FIRE**: guie o torpedo até farmer/torre e mata (Bullet.age arma após 0.35s) · **memória:** [[project_asset_scale_orientation_qa]]
- **Radar plano + abismo vazio-infinito + quips TOD/clima + tutorial no painel + HUD polish (2026-07-13 noite, 10 commits `706e113`→`87661d4`):** **RADAR PLANO**: arrancada a lente esférica/fisheye (`radar_lens`) — anéis/névoa/blips voltam a linear 1:1 (distorção do tamanho/borda confundia), domo→grade equidistante · **glow da HUD** recalibrado 55..190u pra ilha 500u (longe = feixe apontando escorregando na borda, perto = borda toda acende) · **ABISMO = VAZIO INFINITO**: backdrop UNLIT preto a y=-45 sob o void cliff (mata a ClearColor azul vazando pelos buracos do mesh) + `near_abyss` ±2 (tira franja de água costeira do void ring) + **véu do TOD saiu do egui pro SHADER DO CRT** (que agora SEMPRE roda, efeitos CRT→0 quando off) que PULA pixels quase-pretos → o void FURA o véu e lê como nada infinito nas bordas; fix do **jagged**: máscara binária → luminância num cross de 5 taps + `smoothstep(0.006, 0.085)` = fade suave (causa raiz: **o CRT roda ANTES do egui**, o véu tinha que virar termo do shader pra o void sobrepor) · **quips TOD** (a nave anuncia a hora: "meio-dia hora do rango", "ih já é de noite?") + **quips CLIMA** (chuva/neblina/storm/neve/clear em tom cômico) + **quips EMPILHAM** por tamanho (menor no topo/some primeiro, linhas longas embaixo com tempo de ler) · **slider do espantalho** (`scale_scarecrow`, base no chão via `Tower.ground_h`) + **⚖ compare pula pra aba SIZES** · **joystick +10%** (base não vaza ao inclinar) · **label TRACTOR GRAVITON BEAM PULSE** · **play/pause no player de música** (`music_paused` = crossfade pra silêncio) · **tutorial no painel esquerdo** (moldura `hud_panel_h`, auto-contido, `board_res` tuple pro cap 16) · lições: alinhamento `Vec4` no uniform do CRT, cap 16 params (tuple `Res`+`Local`), exe standalone abre sem recompilar (`dynamic_linking` OFF → `game_root` fallback)
- **Migração Bevy 0.15 → 0.19 (2026-07-14, LOCAL-ONLY; `f336764`+`aff4949`+`a1638d5`+`5027cf4`):** upgrade da engine em 4 releases — compila (0 erros), boota, roda **60fps, 0 erros de render**. Deps: bevy 0.19 · rapier3d 0.35 · hanabi 0.19 · egui 0.41 · inspector 0.37. **Event→Message** (0.17): `EventWriter/Reader`→`Message*`, `.send()`→`.write()`, `derive(Event)`→`Message`, `add_event`→`add_message` (65×) · **reorg de crates** (0.17): `NotShadowCaster`→`bevy::light`, `Bloom`→`bevy::post_process`, `ShaderRef`→`bevy::shader`, `RenderAssetUsages`→`bevy::asset`, `HashMap`→`bevy::platform::collections` · **render graph → sistemas na schedule** (0.17): CRT + heat-haze do beam reescritos de `ViewNode`/`add_render_graph_node` pra `add_systems(Core3d, …in_set(Core3dSystems::PostProcess/EarlyPostProcess))` com `ViewQuery`+`RenderContext` params; `RenderPipelineDescriptor` novo (`BindGroupLayoutDescriptor`, `entry_point: Some`, `FullscreenShader` resource, sem `push_constant_ranges`) · **egui 0.32→0.41**: TODOS os 21 sistemas de UI movidos pra **`EguiPrimaryContextPass`** (fontes indisponíveis no `Update` = panic "No fonts until Context::run"), dividindo as tuplas que misturavam gameplay+egui; `try_ctx_mut`→`ctx_mut`, `screen_rect`→`content_rect`, `EguiTextureHandle::Strong`, `painter.rect` ganhou `StrokeKind`, painéis Ui-based (splash com Ui-raiz do contexto), `Frame::none()`→`Frame::NONE` · **componentes** (0.16): `Camera.hdr`→componente `Hdr`, `AmbientLight`/`DirectionalLight` per-camera, `despawn_recursive`→`despawn`, `Timer.finished`→`is_finished`, `Volume::Linear`, `AudioSink::set_volume &mut`, ícone da janela roda no `Update` (`WinitWindows` NonSend só existe pós-criação da janela) · **hanabi 0.14→0.19**: `SpawnerSettings`, `EffectSpawner`, `ParticleEffect` componente · **material shaders `@group(2)`→`@group(3)`** (o material saiu do grupo 2, que virou o mesh storage buffer do GPU-driven rendering — o mismatch storage×uniform crashava o render) · **3 bloqueadores de boot resolvidos em sequência**: `set_window_icon` (NonSend timing) → egui fonts (schedule) → material @group · **JOGÁVEL VALIDADO**: o user jogou até **VITÓRIA (4270 pts)** na 0.19 — loop completo boot→splash→playing→win · **crash do PLAY resolvido** (`a1638d5`): a fita do swirl de vento tinha vértice NaN (`sin(π)` f32 = negativo minúsculo → `.powf(0.8)` NaN; a 0.19 dá panic no `compute_aabb`, a 0.15 tolerava) → `.max(0.0)` no taper · **polish** (`5027cf4`): ícones do cockpit croppados pela silhueta (tamanho uniforme), pause UNIFICADO+centralizado, SIZES com "scarecrow variants" (1º slice do editor 2-4), **FAZENDEIROS AGRESSIVOS** (flee→CHASE, `FarmerState::Chasing`, caçam de 28u carregando PRA nave) · **editor pendente RESOLVIDO** na continuação da sessão (ver bullet abaixo) · **backlog novo que a 0.19 destrava**: contact shadows (resolveria o flicker que motivou os blob shadows), GPU-driven rendering (FPS grama/billboards), vignette/lens built-in, app settings framework · lições: [[project_bevy_0_19_migration]]
- **Editor + MECHA + beam-ovo + bacon (2026-07-14 cont., 13 commits LOCAL-ONLY `1b67fc3`→`f215862`):** **EDITOR fechado** — a causa dos "espantalhos que faltavam" era o library varrer só `textures/env/**`; agora varre `props/**` + `fx/missiles/**` (163→**334 stems**, `ScanRoot` por stem pra a lista de órfãos seguir só do env). Ganhou sz/n por espécie de bicho e pros inimigos (`TOWER_COUNT` const → `cfg.count_shooters`), picker de skin da torre, picker dos 16 torpedos, ⚖ compare pra **qualquer** asset (tamanho vem do manifest ou do quad real de spawn) e a biblioteca `props/` (155 PNGs) navegável/adicionável · **MECHA**: o bicho tinha 3 nomes (tower/scarecrow/espantalho) → todo texto do jogador diz **MECHA** (IDs de achievement preservados); roster = **3 cores do PixaPro** + MIX, tint da "avançada" removido; os mechas certos vieram do PixelLab (eram **objects 8-dir**, não characters — `tools/fetch_mechas.py`) · **HUD BEACON, 3 bugs**: o "retângulo sem máscara" era **premultiplied alpha** (PNG com transparente BRANCO vira soma pura do tint no quad inteiro — `tools/premultiply_hud.py`); o glow era **wash** e não soma (`from_rgba_unmultiplied` interpola; aditivo é alpha 0 + rgb≠0); a **luz direcional nunca existiu** (1 draw por passo vs 8 dirs × 3 anéis). Agora: banda de borda (`tools/make_dash_glow.py`) em **malha tesselada com cor por vértice** — a moldura do cockpit acende e o arco corre por ela (halo POR FORA é cortado pela borda da tela quando o curral fica ao sul) · **SPLASH**: player ⏮⏯⏭ no topo-direita; glifos saíam "?" porque a família nomeada `vt323` não tinha fallback (as padrão têm) · **BEAM-OVO** (ideia do user — a forma do beam vira a seta): 4 iterações até acertar — limaçon (engorda o lado errado) → casco convexo (é um CONE, e o **eixo medial** dele acende como **agulha** no falloff) → **smooth-min de 2 círculos** + **teto radial no brilho** (mata agulha e núcleo fantasma; no círculo o teto não morde → beam sem carga fica idêntico). Motes correm pro bico (`BeamLead`). Armadilha: **`vec3` no meio de uniform `ShaderType`** = encase e naga discordam do offset (vec3 por ÚLTIMO) · **GAMEPLAY**: torpedo solta fumaça ao acertar + 30% mais rápido 1.5s saindo do mecha; **PORCOS** = 10% do rebanho (`CowKind::Pig` no pipeline da vaca), quip "oba, BACON!", **entrega = tanque cheio** (porco ambiente saiu do critters — todo porco visível é abduzível); **burgers** puxados 2.6–4.8× mais forte que o gado, acelerando ao chegar · **UI**: pause centralizado de verdade (**egui não centraliza linha em modo imediato** — medir com o painter e padear), joystick 1.45×, e a **enxurrada de quips** (o asset tour resetava o `WeatherMix` todo frame mas fixava `cfg.weather` só depois de um return antecipado → `weather_fade` via troca de clima a cada frame) · **⚠ ACHADO**: `scatter.json` está **151/151 desligado** desde `0eab074` (13/07) — a ilha não spawna prop nenhum (master toggle salvo por engano); user optou por deixar
- **Neve FALLING-SAND + feixe derrete + HUD + mechas 8-dir + CONFIGS UX (2026-07-15 madrugada, 19 commits LOCAL-ONLY `016361d`→`cf5405c`):** **NEVE VIVA** (ref kandabi/Pixel-Physics-Simulation que o user apontou): v1 rank map de pixels duros (altitude+tempestade+inclinação+dither, quantil = a cobertura É o sorteio 20-33%, NEAREST — a snowline morria em terreno TERRACEADO) → acumula no TAMANHO DO FLOCO (`FLAKE_SIZE=0.22u`, era 1u=ladrilho) → **RUNG 1 falling-sand** (`src/snow_sim.rs`, campo de profundidade vivo ~200² a 7Hz: depósito + relaxação por ângulo de repouso = erosão térmica/grain solver do Houdini + derretimento por altitude; side-view→top-down = a regra do grão vira heightfield; costura de pixel no shader por quadrado-de-floco, sem re-assar) · **FEIXE DERRETE TRILHA**: cápsula contínua (a relaxação fecha o rastro sozinha) → CALOR COM DELAY (não plow instantâneo, o lag é o delay) → falloff no centro (sulco/tigela) → visível (raio 1.1×, centro chega ao chão pelado) · rungs 2 (vento) e 3 (props=colisão) pendentes, user escolheu "sonho completo" · **HUD**: botão do emissor DINÂMICO (acende só com feixe, lente OFF `tools/make_beam_off.py`), contadores na COR DO LED, blips do radar param de LAVAR (flash f⁸, alpha pow(0.6), halo escuro, sat ×1.45), FARMERS AZUL (fonte+LED recolorido no dash_final.png+blip) · **CURRAIS longe da BORDA** (guarda inland via `dist_water`, WANT 16→MIN 9 células; o buraco era o top-up sampler sorteando quadrado sem checar costa) · **MECHAS 8-dir**: viram pra encarar a nave (picker `Direction8::from_velocity`, cada Tower guarda a cor) + DORMEM pro sul e ACORDAM no alcance (WAKE `range+6`/SLEEP `+11`, histerese) · **CONFIGS UX**: pinned ⚖ aparece na SIZES + slider (ScrollArea faltando + `continue` silencioso) · SCROLL global em todas as abas · comparação = LINEUP ÚNICO alinhado pela base (cast+pins, escala honesta tallest→200px) + sem pulo pra SIZES ao marcar · AUTO-SAVE por mudança (debounce 0.6s, compara cfg serializado — change-detection do Bevy é ruído) · comparação em GRID (wrap) · SEARCH 🔍 na ASSETS · porco com `scale_pig` próprio (PIG_CHANCE fixo 10%) · janela redimensionável + ⛶ MAXIMIZAR + 2 sliders na UI: "tamanho dos menus" (`ui_scale`→`set_zoom_factor`) e "tamanho do HUD" (`hud_scale`, `d185871`, multiplica o `dash_scale`/radar/painéis do cockpit, edge-anchored — o que o user queria: "era o HUD mesmo") · **lotes finais** (user "tudo", `e614c95`/`6ea0a0c`/`21f6605`): porcos AMBIENTES de volta (Critter, não abduzível, coexiste com os 10% do rebanho) + radar curral CIANO (era ~igual ao farmer) · **NEVE RUNG 2** deriva por vento (advecção downwind `WIND_ADVECT`, X puro hoje) · **NEVE RUNG 3** props=colisão (campo `blocker`, `snow_stamp_blockers` carimba mechas+currais, os 3 passos do tick respeitam: depósito pula, relaxação=parede, advecção empilha no barlavento — pedras de cenário entram quando scatter religar) = **"sonho completo" fechado** · **decisões resolvidas**: HUD scale, porco ambiente, azul farmer/curral
- **Code review + 2 fixes críticos VALIDADOS (2026-07-16/18, 2 commits LOCAL-ONLY `0781946`+`3dd182b`):** `/code-review` no diff de 07-15 achou: **(1) NEVE CONGELADA** — textura viva `RENDER_WORLD`-only descarrega a cópia CPU após o 1º prepare → `images.get_mut`=None → upload no-op = **causa raiz do "não senti derreter"** (07-15 afinou constantes em cima de upload morto); fix `RenderAssetUsages::default()`, padrão de wind_gpu/hud · **(2) `insert(BeamGrip)` paniqueia** na 0.19 com entidade despawnada no mesmo frame (entrega curral/porco, farmer em pedra; crash.log ×3) — fix `try_insert` em beam.rs + farmer.rs (**1º patch sem grep deixou `farmer_beam` de fora e crashou no teste** — grep TODOS os sites antes de dar fix por pronto); `remove()` = `queue_handled(warn)`, seguro por design · **VALIDADO em jogo** (2 mapas, 12 entregas de curral, 2 fases de neve com acúmulo confirmado no print, 0 panic) · PIPESTATUS mordeu de novo (`cargo run | tail` engoliu exit 101) · handoff queue Phaser limpa (3 done retroativos, 5 abertos, `7cb380f`) · asset "colchão" do print = `tb_truck_teal` (tecla B identifica)
...
- **Roadmap alpha (docs/design/ALPHA_REVIEW.md §7):** ✅quota+escalada → ✅tutorial 10 passos → ✅achievements screen + 5 quests + rare spawns → **PRÓXIMO: bloqueadores técnicos** (AssetPlugin relativo — crítico pra distribuir, panic→log, fullscreen/vsync, reset save) → balance pass (mundo 1× agora!) → zip pros testers
- **Backlog:** wire dos 28 SFX cartoon quando chegarem (6 hooks novos: ufo_hover_loop, moos por estado, bull_snort, farmer_hey, low_fuel, birds/crickets por TOD, ui_click + pitch/vol random) · 5ª faixa de música (player escala sozinho) · cockpit v2 PixelLab REJEITADO (HUD oficial = FinalHud v4 do user) · garagem (coins acumulam de verdade agora via quests) · wave-2 achievements 51-65 (museum_curator já tem infra: save.rare_items) · pool de quests 5→10+ · tema KIT polish (task #31) · crateras por deformação de heightfield (beam cava) · mobile/WASM · grass verlet · vsync toggle · publicar repo Bevy no GitHub (local-only hoje — decisão do user) · **2 menores da review 07-16** (auto-save serializa cfg todo frame mesmo com menu fechado — gatear atrás de `open.0` · advecção de vento com norma L1 >1 na diagonal — dormente enquanto o vento é X puro) · ~~upgrade Bevy 0.15→0.19~~ ✅ **FEITO 2026-07-14** (jogável, vitória validada) · **`scatter.json` 151/151 desligado desde 13/07** — obs 07-18: o manifest LOCAL tem 7 landmarks ligados (4 trucks + windmill + water_tower + old_truck em cena); o resto segue off, ligar quando o user quiser · **pontes voltam quando placement for water-aware** (desabilitadas hoje) · **unbake do balde dentro da arte do curral** (só toggle não remove — pixel baked; oferecido, user não pediu ainda) · validação user pendente: habitats/torpedos/beam-bússola/HUD v4 no pixel/caça ao tesouro das quests/aba AWARDS de 07-13 + clusters/ondas direcionais de 07-12 + escala rel/abs/orientação/aba ASSETS animals-enemies/friendly-fire de 07-13 + radar plano/abismo vazio-infinito+fade suave/quips TOD+clima+empilhamento/tutorial no painel esq/play-pause música/joystick+10%/label graviton de 07-13 noite (abertas: 4a — player de música no menu vs no HUD perto do SCORE, user confirma; jagged pode pedir rampa/raio maior no cross) + **validação pendente da continuação 07-14**: mechas novos no pixel · beam-ovo (forma/motes) · porcos-bacon · burgers rápidos · pause centralizado · joystick 1.45× · **NEVE FALLING-SAND completa** — rung 1 (acúmulo) + rung 2 (vento, `6ea0a0c`) + rung 3 (props=colisão, `21f6605`) FEITOS ("sonho completo"); pedras de cenário entram no blocker quando `scatter.json` religar · **validação 07-15**: neve viva ✅ VALIDADA 07-16/18 (acúmulo no print do user, 2 fases de neve, 0 panic) · feixe derretendo trilha (delay/sulco) SEM confirmação explícita · HUD (botão beam dinâmico, contadores/blips, farmer azul) · mechas 8-dir dormindo/acordando · CONFIGS UX (pinned na SIZES+slider, scroll global, lineup base, auto-save, grid, search, porco size, janela maximizável, ui_scale menus + hud_scale cockpit) · **decisões 07-15 RESOLVIDAS**: porco ambiente que-não-pega → SIM (`e614c95`, coexiste) · azul farmer vs curral → curral virou ciano (`e614c95`)
...
### ✅ Pronto (cont. — sessão 2026-08-29 · 9 faixas de música + player 1:1 do Bevy + 3 bugs de áudio)
- **9 faixas** (eram 4 no disco, 3 tocáveis) — 5 novas vieram como `.mp4` (vídeo 1024² + AAC), áudio extraído pra MP3 192k com ffmpeg. As 4 antigas batem duração com os `.mp4` entregues (mesmos masters, não reencodadas). Copiadas pros DOIS builds; o Bevy varre `assets/audio/*.mp3` no boot, então lá já aparecem no player sozinhas
- **POOLS por estado** (menu 2 · dia 5 · noite 2) substituem as 3 faixas fixas: troca de estado sorteia sem repetir, faixa toca com `loop:false` e **ao terminar sorteia a próxima do mesmo pool**. `_ensureTrack` baixa UMA por vez (28MB não cabem no boot do Pages); listener global `filecomplete` (vazava por restart) → `filecomplete-audio-<key>` com `once()`. `Barnyard_UFO` estava no disco desde 22/07 sem nunca tocar
- **Aba AUDIO no CONFIGS** — port 1:1 do `MenuTab::Audio` do Bevy: sliders sfx/music + player (play/pause · ⏮ · ⏭ · combo de faixa sobre `['auto', ...9]`). Defaults alinhados ao `DebugConfig` do Bevy (**sfx 0.8**, **music 0.55** — eram 0.9/0.7 hardcoded). Achievement `audiophile` destravado (era dormente por falta do player)
- **Transport no SPLASH** (⏮ ⏯ ⏭ topo-direita + nome da seleção à esquerda) — port do bloco "MUSIC TRANSPORT, TOP-RIGHT" do `gameflow.rs`, mesmas medidas/paleta e a regra de que **pular faixa despausa**. Entra no `allBtns`, some no start. Glifos com `fontFamily: 'VT323, monospace'` — a família nomeada sozinha dá tofu
- **BUG: música de menu NUNCA tocou** — `_updateAudio` vivia no `_updateBody`, e o `update()` retorna cedo no splash (`!gameStarted`) e no game over. Pool de menu era código morto e a música não sumia no fim de partida. Subiu pro topo do `update()`
- **BUG (o maior): o lerp de volume nunca convergia** — no Phaser 3.60 o getter `sound.volume` **não reflete** o `setVolume()` na hora (lê valor sincronizado no tick). Provado ao vivo: `add(key,{volume:0})` → `.volume === 1` com `currentConfig.volume === 0`. Todo lerp `s.setVolume(s.volume + (target - s.volume)*k)` lia 1 e escrevia sempre o mesmo → **faixa entrava quase no volume cheio e travava em ~0.95** (medido 0.949/0.977) em vez de 0.55; valia pros loops beam/chuva/vento também. Volume atual passou a morar em `_audio.vol`
- **BUG: `pointerdown` dobrado** (touch + mouse emulado) fazia o play/pause ligar e desligar no mesmo toque → gate de 250ms
- **`dbg.audio` fora do merge do `_loadDebugCfg`** (o merge é explícito por seção) — a seção ficaria `undefined`
- **Ambiente, pra não cair de novo:** pane oculto **congela o rAF** e o loader do Phaser parece travado (inflight 0, dezenas PENDING) — não é bug, screenshot força pintura e drena · pane 800×450 dispara o **modo mobile** (preloader vermelho), emular 1280×800 antes de testar · `github.io` responde **301 pro domínio `zero-onebit.com`**
- **Gap conhecido:** `fall.wav` (`Sfx::Fall`) é o único SFX do Bevy sem par — dispara no `AbyssFall`, mecânica nunca portada. E o **AUTO do Bevy ainda roteia só 3 das 9** (o web passou na frente)

### 🚧 Em andamento
- **⚠️ VALIDAÇÃO DO USER PENDENTE (o gargalo real)** — 07-22, 08-10 e 08-29 foram validados por **estado lido no console** (e, em 08-29, cliques reais nos botões), nunca jogado nem **ouvido**. Sem prova a olho/ouvido: **áudio inteiro** (o pane do browser silencia — o jogo nunca foi escutado; e o bug do lerp de volume mostra que dá pra passar meses com o som errado sem ninguém notar), paleta do terreno canvas, cockpit no pixel, tutorial 09/10 jogados de verdade
...
3. **Ouvir o jogo** — o áudio nunca passou por um par de ouvidos; é o teste que sobrou depois de 08-29 (mix das 9 faixas, ganhos dos SFX, crossfade). Opcional em cima disso: passar os pools pro AUTO do Bevy (hoje 3 de 9)
